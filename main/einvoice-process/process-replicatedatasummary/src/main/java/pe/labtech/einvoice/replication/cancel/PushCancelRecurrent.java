...
public class PushCancelRecurrent extends AbstractRecurrentTask<DocumentData> {
    ...
    public void init() {
        ...
        this.findTasks = () -> privateManager.seek(e -> e
                .createQuery(
                        "SELECT o FROM DocumentData o WHERE o.document.documentNumber LIKE 'RA%'AND o.replicate = TRUE AND o.data <> NULL",
                        DocumentData.class
                )
                .getResultList()
        );
        ...
        this.consumer = t -> manager.handle(e -> {
            ...
            e
                    .createQuery("UPDATE CancelHeader d SET d." + targetField + " = :param WHERE d.id = :id")
